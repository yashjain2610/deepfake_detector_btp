...
(ii)~\emph{Cross-dataset generalization}---detectors overfit to dataset-specific artifacts (e.g., blending boundaries, colour distributions) and fail when exposed to unseen generators or post-processing pipelines~\cite{li2020facexray}.
...
Multi-Attentional~\cite{zhao2021multiattentional} localizes tampered regions with texture attention.
RECCE~\cite{cao2022recce} employs reconstruction-classification learning.
...
LAA-Net~\cite{nguyen2024laanet} localizes artefacts for quality-agnostic detection.
...
\begin{figure*}[t]
  \centering
  \begin{tikzpicture}[
    block/.style={draw, rounded corners, minimum height=0.75cm, minimum width=1.7cm,
                  align=center, font=\small},
    wideblock/.style={draw, rounded corners, minimum height=0.75cm, minimum width=2.4cm,
                  align=center, font=\small},
    arrow/.style={-{Stealth[length=2mm]}, thick},
    x=1cm, y=1cm,
  ]
    %% ---- Input ----
    \node[block, fill=blue!12] (input) at (0,0) {Face Crop\\$299{\times}299{\times}3$};

    %% ---- Spatial stream (top row) ----
    \node[block, fill=orange!20] (xception) at (2.6, 1.1) {Xception\\(ImageNet)};
    \node[block, fill=orange!12] (s_proj)   at (4.9, 1.1) {Proj.\ 2048$\!\to\!$512};

    %% ---- Frequency stream (bottom row) ----
    \node[block, fill=green!18] (dct)    at (2.0,-1.1) {Per-Ch.\ DCT};
    \node[block, fill=green!18] (resnet) at (4.0,-1.1) {ResNet-18\\(ImageNet)};
    \node[block, fill=green!12] (f_proj) at (6.2,-1.1) {Proj.\ 512$\!\to\!$512};

    %% ---- Gate ----
    \node[block, fill=red!12] (gate) at (7.6, 0) {Gate\\$\sigma$(MLP)};

    %% ---- Fusion ----
    \node[wideblock, fill=purple!12] (fusion) at (9.6, 0)
      {$g\!\odot\! z_s\!+\!(1\!-\!g)\!\odot\! z_f$};

    %% ---- Classifier ----
    \node[block, fill=gray!18] (mlp) at (11.6, 0) {MLP\\$512\!\to\!256\!\to\!1$};
    \node[block, fill=yellow!18] (out) at (13.3, 0) {Real /\\Fake};

    %% ---- Arrows: input -> streams ----
    \draw[arrow] (input.north) |- (xception.west);
    \draw[arrow] (input.south) |- (dct.west);

    %% ---- Arrows: spatial stream ----
    \draw[arrow] (xception) -- (s_proj);
    \draw[arrow] (s_proj.east) -| ([xshift=-0.25cm]gate.north);

    %% ---- Arrows: frequency stream ----
    \draw[arrow] (dct) -- (resnet);
    \draw[arrow] (resnet) -- (f_proj);
    \draw[arrow] (f_proj.east) -| ([xshift=0.25cm]gate.south);

    %% ---- Arrows: gate -> fusion (gate provides g; dashed carry z_s, z_f) ----
    \draw[arrow] (gate) -- (fusion);
    \draw[arrow, dashed, gray] (s_proj.east) -- ++(0.3,0) |- (fusion.north west);
    \draw[arrow, dashed, gray] (f_proj.east) -- ++(0.3,0) |- (fusion.south west);
    ...
    %% ---- Stream labels ----
    \node[font=\footnotesize\itshape, text=orange!70!black] at (3.75, 1.65)
      {Spatial stream};
    \node[font=\footnotesize\itshape, text=green!50!black] at (4.1, -1.65)
      {Frequency stream};
  \end{tikzpicture}
  \caption{Dual-stream architecture. The spatial stream (Xception, 2048-D$\to$512-D) and the frequency stream (ResNet-18 on per-channel DCT maps, 512-D) are projected to a common 512-D space. Their concatenation drives a gating MLP that produces $g\in(0,1)^{512}$; the element-wise fused vector $z$ is classified by a two-layer MLP head.}
  \label{fig:arch}
\end{figure*}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figs/ablation_bar.png}
  \caption{Ablation: AUROC and balanced accuracy for each fusion variant on FF++ validation (video-level).}
  \label{fig:ablation}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Cross-dataset AUC (\%) for FF++$\to$Celeb-DF-v2. Numbers are drawn from respective publications; evaluation protocols vary.}
\label{tab:sota}
\begin{tabular}{lcc}
\toprule
